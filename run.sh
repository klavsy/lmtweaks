...
# 7. OPTIMIZĀCIJA (ZSWAP & SWAPPINESS)
print_step "Atmiņas Optimizācija (Zswap)"
...
if [ "$TOTAL_RAM_GB" -le 8 ]; then
    log "RAM <= 8GB. Zswap 40%, Swappiness 30."
    NEW_GRUB_LINE='GRUB_CMDLINE_LINUX_DEFAULT="quiet splash zswap.enabled=1 zswap.max_pool_percent=40 zswap.zpool=zsmalloc zswap.compressor=lz4"'
    SWAPPINESS_VAL=30
else
    log "RAM > 8GB. Zswap 20%, Swappiness 60."
    NEW_GRUB_LINE='GRUB_CMDLINE_LINUX_DEFAULT="quiet splash zswap.enabled=1 zswap.zpool=zsmalloc zswap.compressor=lz4"'
    SWAPPINESS_VAL=60
fi

if grep -q "GRUB_CMDLINE_LINUX_DEFAULT" "$GRUB_FILE"; then
    sed -i 's/^GRUB_CMDLINE_LINUX_DEFAULT=.*/'"$NEW_GRUB_LINE"'/' "$GRUB_FILE"
    update-grub
fi
...
log "Konfigurē Firefox/LibreWolf (SSD Saver + No-AI)..."
...
# UI: WINDOWS 11 STYLE (Cinnamon Only)
if [ -f "/usr/bin/cinnamon-session" ]; then
    log "Konfigurē Cinnamon (Windows 11 stilā)..."
...
print_step "Pabeigts!"
echo -e "${GREEN}Sistēma konfigurēta (v35 - CPU Turbo).${NC}"
echo "CPU: Performance mode."
echo "Disk: Write buffers optimizēti."
echo "Atmiņa: Zswap aktivizēts."
